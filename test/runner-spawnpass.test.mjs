...
test("spawnPass sums routed verdicts from JSON-line stdout, tolerating non-JSON lines", async () => {
  const child = fakeChild();
  const spawn = () => child;
  const p = spawnPass({ passPath: "/fake/pass.mjs", env: {}, spawn });
  queueMicrotask(() => {
    child.stdout.emit("data", Buffer.from(JSON.stringify({ phase: "reconcile", routed: 3 }) + "\n"));
    child.stdout.emit("data", Buffer.from("not json, just a log line\n"));
    child.stdout.emit("data", Buffer.from(JSON.stringify({ phase: "reconcile", routed: 4 }) + "\n"));
    child.emit("close", 0, null);
  });
  const result = await p;
  assert.deepEqual(result, { ok: true, verdicts: 7, error: undefined });
});
