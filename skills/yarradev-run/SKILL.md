...
| Verdict payload (`routeVerdict`) | the status is routable but the field its branch needs is absent: `advice`/`clean`/`veto`/`hold` without `head`, `submitted` without a complete `evidence{repo, pr_number, head}` | act **not** dispatched; **CLEAR_LEASE**; consume; **escalate** naming the missing field; outcome `malformed_verdict` (GH #94). Before this, the act was dispatched anyway, the script rejected its own args with exit 2, and that was misread as a transient failure — so the verdict was retried every pass **forever**, unconsumed, lease held. Note `role` is **not** required on advisor verdicts: `routeVerdict` sources it from `ctx.role`. |
| Any act script | exits non-zero with no JSON on stdout | exit **2** = it rejected its arguments → deterministic (`bad_invocation`) → **park**, never retry. Any other exit = crash/degraded board → transient → retry next pass. `emit()` only ever exits 0/1, so exit 2 is unambiguous. |
| Any transition act | 422 `gate_blocked` | **Never parks (GH #100).** A blocked gate is the board reporting "not satisfied *yet*" — the normal state of a card that just arrived at a stage. **CLEAR_LEASE**; `decide` re-derives next pass; reconcile outcome `gate_blocked`, naming the failing predicates from `blocked_by`. Parking on it was self-sustaining: the park sets `blocked=true`, `not_blocked` is itself a gate predicate, and `list-ready` skips parked cards — so the stage owner was never re-dispatched and the missing evidence could never be produced. It also made transient conditions terminal (one CI flake = permanent park). If the predicates never change, that is a **board config** problem (e.g. `tests_green{check:"e2e"}` with no such check in CI) — the named predicates are how you see it. `advisor_clear` keeps its own reshape path. |
